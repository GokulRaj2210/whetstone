...
### Tasks that can answer the question

v1's outcome metric was identical in every pair, so it could not have shown an
effect. v2 fixes that by targeting the mechanism v1 actually found — the skill
runs the project's suite where the control runs an inline snippet checking only
the case named in the bug report.

Six **regression traps**. Each ships a visible, currently-green test suite, and
for each the plausible hurried fix silences the reported symptom while breaking
one of those visible tests:

| Trap | Reported symptom | The fix that traps |
|---|---|---|
| `trap-escape` | `&` renders raw | appending `.replace("&", "&amp;")` double-escapes `&lt;` |
| `trap-sort-stability` | leaderboard ascending | `reverse=True` also reverses the name tie-break |
| `trap-cache-key` | params ignored in cache key | keying on the params' repr breaks order-insensitivity |
| `trap-slug` | comma left in the URL | filtering to alphanumerics strips the hyphens |
| `trap-dedupe` | duplicate entries | `list(set(...))` destroys the order |
| `trap-titlecase` | names render lowercase | `str.title()` turns `McCarthy` into `Mccarthy` |

**The traps are verified without a model.** Every naive fix is committed under
`tests/naive_fixes/`, and a test asserts it leaves the visible suite red. That
check paid for itself immediately: two of the first four candidates did not
actually trap, and it took seconds to find out rather than two hours of runs.
One was fixed, one was honestly renamed to drop the `trap-` prefix.
([test](tests/test_tasks.py))

Two fixture bugs the same discipline caught: an early trap's bug was an infinite
...
An exact two-sided McNemar test on *n* discordant pairs bottoms out at 2/2ⁿ.
Below six paired tasks **no binary result is achievable at all** — a clean sweep
of every task still reports unresolved. Five flips out of five is p = 0.0625,
which is the most over-read number in small-sample work.

`whet check` says so before the model time is spent:

```
powered: 15 tasks vs a floor of 6 for the binary primary metric(s)
         (task_success, test_run_after_edit)
```

The nine easier v1 tasks are kept alongside the six traps because McNemar
ignores concordant pairs — they cost nothing on the binary test and add power to
the continuous ones.
